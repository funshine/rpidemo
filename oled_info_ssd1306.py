...
while True:
    ...
    draw.text((x, top + 14), 'disk:' + diskinfo() +
              '  RAM:' + raminfo(), font=font, fill=255)
    draw.text((x, top + 24), 'temp:' + cputemp() +
              'C  CPU:' + cpuinfo(), font=font, fill=255)
    draw.text((x, top + 34), 'signal:' + '-60' + 'dBm', font=font, fill=255)
    draw.text((x, top + 44), 'LAN:' +
              get_ip_address('eth0'), font=font, fill=255)
