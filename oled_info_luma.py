        ...
        while True:
            for x_info in range(40):
                with canvas(device) as draw:
                    ...
                    top = padding
                    x = padding
                    draw.text((x, top), time.strftime(" %Y-%m-%d %H:%M:%S ",
                                                    time.localtime(time.time())), font=font, fill=255)
                    # draw.text((x, top+14), platform.system() + ' ' + platform.release(), font=font2, fill=255)
                    draw.text((x_info, top + 14), mc.info, font=font2, fill=255)
                    draw.text((x, top + 24), 'disk:' + diskinfo() +
                            '  RAM:' + raminfo() + ' ', font=font2, fill=255)
                    draw.text((x, top + 34), 'temp:' + cputemp() +
                            'C  CPU:' + cpuinfo() + ' ', font=font2, fill=255)
                    # draw.text((x, top+44), 'signal:' + '-60' + 'dBm', font=font2, fill=255)
                    draw.text((x, top + 44), 'LAN:' +
                            get_ip_address('eth0'), font=font2, fill=255)
